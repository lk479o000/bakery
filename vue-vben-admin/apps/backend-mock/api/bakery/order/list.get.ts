...
export default defineEventHandler((event) => {
  ...
  const { type, status, orderNo, page = 1, pageSize = 10 } = query;
  let filtered = [...orders];
  if (type) filtered = filtered.filter(o => o.type === type);
  if (status !== undefined) filtered = filtered.filter(o => o.status === Number(status));
  if (orderNo) filtered = filtered.filter(o => o.orderNo.includes(orderNo as string));
